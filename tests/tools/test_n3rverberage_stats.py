...
def _tools_path() -> Path:
    return Path(__file__).resolve().parents[2] / ".opencode" / "tools" / "n3rverberage-stats.ts"
...
class TestN3rvStatsTools:
    ...
    def test_exports_memory_stats(self) -> None:
        content = _tools_path().read_text(encoding="utf-8")
        assert "export const n3rverberage_memory_stats" in content
    ...
    def test_exports_hub_health(self) -> None:
        content = _tools_path().read_text(encoding="utf-8")
        assert "export const n3rverberage_hub_health" in content
    ...
    def test_check_pending_tasks_falls_back_to_env(self) -> None:
        content = _tools_path().read_text(encoding="utf-8")
        assert "N3RVERBERAGE_AGENT_SOURCE" in content or "agent_id" in content
