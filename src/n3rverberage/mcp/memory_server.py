...
logger = __import__("logging").getLogger("n3rverberage.mcp.memory")

_SEARCH_NUDGE_THRESHOLD = 3


def build_memory_server(project_root: Path | None = None):
    """Build and return the n3rverberage-memory MCP server."""
    ...
    profile = os.environ.get("N3RVERBERAGE_MEMORY_PROFILE", "full")
...
def run_memory_server() -> None:
    """Entry point for n3rverberage-memory subprocess."""
    build_memory_server().run()


def main() -> None:
    """Entry point for n3rverberage-memory command."""
    run_memory_server()
